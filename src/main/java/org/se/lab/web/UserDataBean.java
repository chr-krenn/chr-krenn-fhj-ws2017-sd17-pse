...
public class UserDataBean implements Serializable {
		...
		if (session.size() != 0 && session.get("user") != null) {
			...
			if (userProfId != null && !userProfId.equals("null")) {
				...
				for (User u : service.getContactsOfUser(loggedInUser)) {
					// Wenn sich der User des aktuell angezeigten Profils in der Kontaktliste
					// befindet wird der removeBtn angezeigt
	...
	public void addContact() {
		...
		String contactName = FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap()
				.get("contactName");
